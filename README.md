...
Pages 构建还会从 `src/data/site/infra.ts` 生成 `public/infra-status.json`：GitHub Actions 服务端按最终 HTTP 状态探测并写入快照，前端优先读取同源快照，代理不可用时再回退到代理或浏览器直连。工作流每 15 分钟运行一次；手动运行 `pnpm infra:status:sync` 可刷新快照。不支持 CORS 且没有可用快照或代理的目标仍会显示 offline。

...
const checks = [
  ...
  ['template deploy refreshes the Infra snapshot', /schedule:\s*\r?\n\s*- cron:/.test(deployWorkflow) && deployWorkflow.includes('pnpm run build')],
  ['template package generates the Infra snapshot', packageJson.scripts?.['infra:status:sync'] === 'node scripts/generate-infra-status.mjs' && packageJson.scripts?.build?.includes('infra:status:sync')],
  ['template README exported', rootReadme.includes('Vorlage') && rootReadme.includes('https://github.com/dieWehmut/Vorlage')],
  ['README documents automatic Pages base and Infra probe', rootReadme.includes('GITHUB_REPOSITORY') && rootReadme.includes('BASE_PATH') && rootReadme.includes('VITE_INFRA_PROBE_URL') && rootReadme.includes('HTTP') && rootReadme.includes('`200`') && rootReadme.includes('SSRF') && rootReadme.includes('infra-status.json') && !rootReadme.includes("const base = '/Vorlage/'")],
  ...
  ['translated READMEs document Infra probe', translatedReadmeSources.every((source) => source.includes('VITE_INFRA_PROBE_URL') && source.includes('HTTP') && source.includes('`200`') && source.includes('SSRF') && source.includes('infra-status.json'))],
